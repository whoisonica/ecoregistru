...
public enum ErrorMessageEnum {
    ...
    WEIGHING_OPERATION_TYPE_UNAVAILABLE("weighing.operation.type.unavailable", "Deocamdată se înregistrează intrări, ieșiri și transferuri; inventarul și sortarea vin mai târziu."),
    ...
    RECEIVED_FORM_DATE_REQUIRED("received.form.date.required", "Scrie data la care ai primit formularul."),
    RECEIVED_FORM_NUMBER_REQUIRED("received.form.number.required", "Scrie numărul formularului, cum e tipărit pe el."),
    RECEIVED_FORM_SENDER_REQUIRED("received.form.sender.required", "Scrie cine a trimis marfa (expeditorul de pe formular)."),
    RECEIVED_FORM_KIND_UNKNOWN("received.form.kind.unknown", "Formularul e Anexa 3 (nepericuloase) sau Anexa 2 (periculoase)."),
    RECEIVED_FORM_QUANTITY_NOT_POSITIVE("received.form.quantity.not.positive", "Cantitatea trebuie să fie mai mare decât zero."),
    RECEIVED_FORM_REASON_REQUIRED("received.form.reason.required", "Scrie de ce corectezi rândul: registrul păstrează și rândul greșit."),
    RECEIVED_FORM_ALREADY_CORRECTED("received.form.already.corrected",
            "Rândul ăsta a fost deja corectat. Dacă și corectura e greșită, corecteaz-o pe ea."),
    // D2.5 — transferul între depozitele firmei.
    TRANSFER_TARGET_REQUIRED("transfer.target.required", "Alege depozitul în care pleacă marfa."),
    TRANSFER_SAME_DEPOT("transfer.same.depot", "Transferul pleacă dintr-un depozit în altul: alege alt depozit de destinație."),
    TRANSFER_NO_COUNTERPARTY("transfer.no.counterparty",
            "Transferul rămâne în firmă: nu are partener, persoană fizică sau plată. Pentru o vânzare, fă o ieșire."),
    TRANSFER_TARGET_NOT_AUTHORIZED("transfer.target.not.authorized",
            "Depozitul de destinație n-are o autorizație de mediu valabilă la data transferului. Deșeurile se transportă numai la un amplasament autorizat (HG 1061/2008 art. 1 alin. (3)). Completează autorizația în Setări → Puncte de lucru."),
    TRANSFER_NOT_IN_TRANSIT("transfer.not.in.transit", "Se recepționează doar un transfer plecat și încă nerecepționat."),
    TRANSFER_RECEIVED_ON_REQUIRED("transfer.received.on.required", "Scrie data la care a ajuns marfa."),
    TRANSFER_RECEIVED_BEFORE_DEPARTURE("transfer.received.before.departure", "Marfa nu poate ajunge înainte să plece. Verifică data recepției."),
    TRANSFER_RECEIPT_LINES_MISMATCH("transfer.receipt.lines.mismatch",
            "Recepția are câte un rând pentru fiecare sortiment plecat — nici mai multe, nici mai puține."),
    TRANSFER_DIFFERENCE_NEEDS_NIR("transfer.difference.needs.nir",
            "Diferența dintre cântăriri e peste toleranța celor două cântare. Scrie numărul NIR-ului și decizia comisiei."),
