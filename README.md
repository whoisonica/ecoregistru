...
| **The aviz** | **Evidențe** → *Predări* → "Aviz de însoțire" on any handover | A new tab with the delivery note (*aviz de însoțire a mărfii*): header, sender, recipient, transport, the waste line and two signature boxes. The driver prints as "name \| CNP \| ID"; the note's number is the movement's document reference |
...
| **The centralised evidence** | **Evidențe** → "Evidența gestiunii deșeurilor centralizată" (formerly "Declarația anuală") | The centralizator: one line per waste code — opening stock, generated, recovered, disposed, closing stock, and through whom — one page per work point. A row whose exits carry no R/D code is marked `(*)` on the stock, with the reason under the table |
| Control dossier | **Dosar de control** → download | Files numbered in reading order: `00-cuprins.txt` (names the 15 March deadline), `01-evidenta-gestiunii-deseurilor-«year».pdf` — the same four-chapter sheet — `02-evidenta-centralizata-«year».pdf`, then `03-anexa1-ambalaje-«year».xls/.pdf` for a producer or importer, the Anexa 3 pairs, and `NN-autorizatii-parteneri.pdf` (landscape, diacritics, coloured status, waste codes per partner, page numbers). The working tables sit in `90-de-lucru/`, the attachments in `99-documente-justificative/` |
| Three years of dossier | **Dosar de control** → *Perioada* → „Ultimii 3 ani" | One folder per year (`2024/`, `2025/`, `2026/`), `01-autorizatii-parteneri.pdf` once at the root, and a `00-cuprins.txt` that names any year with no evidence lines instead of shipping a blank sheet |
...
| AFM contributions | **Clienți** → edit a company | Monthly 2% and annual packaging only; "Economia circulară" (landfills) is gone from the form, the server and the deadlines (`V60`) |
...
| Filling a partner from its CUI | **Parteneri** → "Adaugă partener" → type the CUI → "Completează din ANAF" (the same button sits next to the CUI on **Clienți** and on a consultancy) | Name, registered address and trade register number come from ANAF's public taxpayer registry (web service v9) into the rubrics that are still **empty** — nothing typed is overwritten, because the name you use and the unloading address may differ from the registry. An inactive taxpayer is flagged. Signed-in users only; calls are spaced one a second, as ANAF's documentation asks, and a company found is kept for a day. ANAF down answers `503`, and the form says to type the data by hand |
