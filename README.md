...
| Packaging declaration | **Generare** → *Ambalaje* → "Descarcă Anexa 1 Ambalaje" → ".xls" | Two sheets, `Tabelul nr. 1` and `Tabelul nr. 2`, at the same cell addresses as the model. Table 1 is summed from the movements; the material gives the row, the kind of packaging gives the column. The tab is the third one on **Generare** — Anexa 1 Ambalaje is summed from the company's own waste, which is exactly that screen's register; a company without **Generare** (a pure collector) keeps `/ambalaje` as a screen of its own |
