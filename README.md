...
| Filling a partner from its CUI | **Parteneri** → "Adaugă partener" → type the CUI → "Completează din ANAF" | Name, registered address and trade register number come from ANAF's public taxpayer registry (web service v9) into the rubrics that are still **empty** — nothing typed is overwritten, because the name you use and the unloading address may differ from the registry. An inactive taxpayer is flagged. Signed-in users only; calls are spaced one a second, as ANAF's documentation asks, and a company found is kept for a day. ANAF down answers `503`, and the form says to type the data by hand |
