...
/mobile     Expo SDK 57 · React Native — field app, early (not in the stores)
/scripts    Deploy through the split repos, client data export, demo seeding
/docs       Regulatory research, primary sources, interface rules, delivery log
```
...
friendlier SaaS-looking pass was rejected: a graphite side panel whose rows carry a key, a name and a live indicator — with the open screen's tabs listed under
its row (`frontend/src/lib/screenTabs.ts`, read by both the page and the panel) — the page
...
their own routes (`/generare`, `/intrari`, `/iesiri`) and server-side totals above each list. The month display, the
large add keys and the search row the panel first shipped with were removed by the owner on 18.09.2026 and are not to
come back; a working screen must fit 1440 × 900 without scrolling. Screen copy names what the person does before the article
...
The dev seed is deliberately rich: 3 work points, 6 partners and 29 movements across 7 months (February–August
2026), so the cumulative stock actually carries over and the evidence screens have something to show. A second
tenant — a pure generator, "Proba Automata SRL" — is seeded next to it, so that what one company type must not see is
proved on a real account rather than assumed.
...
| Origin on the art. 48 record | **Intrări** or **Ieșiri** → "Evidența cronologică" | An "Originea" column on takeovers: "populaţie" for an individual, otherwise what the operation or the partner says. Nobody classified it → the cell stays empty |
...
| Packaging register | **Generare** → *Mișcări* → the packaging keys | Since 18.09.2026 there is one movement list, not two: the keys above it — **Ambalaje**, **Ambalaj pus de noi pe piață**, **De completat** — ask the server (`?packaging=ANY|ON_MARKET|INCOMPLETE`) and the filtered rows say, under the waste code, what each line is missing. Record one with no material chosen on `15 01 04` and it reads "Fără material": the quantity stays out of the declaration until someone says aluminium or steel |
...
| Adding a partner | **Parteneri** → "Adaugă partener" | Four steps with a rail on the left, instead of a column of fifteen blocks: about the partner (the CUI first, with ANAF) · what it does for you (type, who hauls, who invoices whom) · the environmental authorization · its work points and drivers. "Continuă" checks its own step; when editing, "Salvează" sits on every step. All four steps stay mounted, so nothing typed is lost between them |
...
| Why nothing on screen shows stock | **Acasă**, **Generare** | A generator owns no weighbridge and keeps no stock: the waste sits in the bin until the collector comes, and the quantity is learnt at handover, from their ticket. So the server refuses a generation without an exit (`V58`), the generation is the figure *derived* from the handover (`V24`), and `stock = previous + generated − recovered − disposed` comes out zero by construction. The dashboard tile went on 16.09.2026 and the column on 18.09.2026. "Left in stock" survives where the law puts it: on the printed Anexa 1 (cap. 1) and on the centralizator |
