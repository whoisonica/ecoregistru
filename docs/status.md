...
> **20.09.2026, noaptea — ✅ cele cinci observații de pe producție, BUG-053 și G06** (nedeployat la scrierea intrării; fără migrații, schema rămâne **V63**, liberă **V64**). Măsurat pe rulare curată, pe tot ce cuprinde intrarea (inclusiv „PASTY", adăugat mai jos): backend **1058 de teste / 132 de clase, 0 căzute, 5 sărite**; `npm test` **67/67**; `tsc` curat; `npm run build` curat, bundle **997 kB** (gzip 284). *(Cifrele scriau 1057 și 66/66 — erau măsurate înainte de paragraful „PASTY", care și-a adus proba lui de fiecare parte; remăsurat pe 20.09 la prânz, pe `0f1455e`.)*
>
