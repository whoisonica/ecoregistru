...
| `Reception` / `Delivery` | ✅ **schemă + entități (Etapa 2a)** — marfa tranzacționată, flux separat de `WasteMovement`, alimentează registrul art. 48, **NU** Anexa 1. Recepția e documentul primar (are și prețul, pentru contribuția AFM de 2%) | ecrane, servicii, controllere — **Etapa 8**; mișcările `COLLECTED` vechi se mută fizic acolo atunci, o singură dată. 🔴 **Formatul de ieșire lipsește:** art. 48 descrie conținutul, nu forma, și n-are anexă cu facsimil — trebuie cerut un model completat (întrebarea AD) | mare |
| — | ✅ **conversia kg → tone există din 04.09.2026**, într-un singur loc (`frontend/src/lib/units.ts`), și se vede pe ecranul Evidențe ca total anual per cod. Fișa de gestiune și declarația de ambalaje **rămân în kg pe hârtie** — acolo sunt corecte, iar un „document de depunere” nou ar fi fost un format oficial inventat | exportul în tone al registrului art. 48 rămâne la **Etapa 8**, cu aceeași conversie | ✅ partea vizibilă |
