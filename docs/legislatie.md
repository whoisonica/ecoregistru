...
| `Partner` (colector/transportator + autorizație) | operatorul din Cap. 3/4 ✅; tipul e `GENERATOR`/`COLLECTOR` (`V11`), cu `RECOVERER` adăugat pe 24.08 fiindcă decide caseta „Destinat:" de pe Anexa 3; rolul comercial e două flaguri, iar **transportatorul e o bifă, nu un tip** (`V28`, care a făcut și `type` nullable — o firmă de transport pură nu face nimic cu deșeul) — aceeași firmă e des și colector, și transportator | 🟠 **viza anuală nu e în model.** Art. 34¹ (nou, 11.07.2026) obligă ANMAP să publice lista operatorilor autorizați cu **CUI**, autorizația integrală și **statusul vizei anuale**. O autorizație poate fi în termen și viza refuzată — caz în care `authorizationExpiry` spune „e bună" și nu e. Plus: nimic nu compară azi autorizația cu registrul public, deși CUI-ul e cheia gata făcută. `surse-oficiale.md` §2.6 · `todo-lansare.md` **P3.9** | 🟠 medie |
...
| **Buletinele de analiză** — caracterizarea deşeului periculos, art. 8 alin. (4) şi art. 48 alin. (2) | `AnalysisBulletin` + `AnalysisBulletinService` (`V38`), legate de **cod**, nu de mişcare; dosarul de control le împachetează şi **constată per cod** ce lipseşte | ✅ *(11.09.2026, `V38`)* |
| Încadrarea ca nepericulos la un **cod-oglindă**, art. 8 alin. (2) | `waste_codes.mirror_of` (`V37`) + badge „Cod-oglindă"; se stinge cu **buletin pe cod** sau cu ataşament pe mişcare — actul numeşte două feluri de dovadă | ✅ *(11.09.2026)* |
